...
class OpenCDInferencerFeature(MMSegInferencer):
    ...
    def visualize(self,
        ...
        for single_inputs, pred in zip(inputs, preds):
            ...
            #     drawn_img_pre = self.visualizer.draw_featmap(
            #         feature, img_from_to[0], channel_reduction='select_max')
            #     self.visualizer.add_image(f'{img_name[:-4]}_pre_feature_map{i}', drawn_img_pre)
            #     drawn_img_post = self.visualizer.draw_featmap(
            #         feature, img_from_to[1], channel_reduction='select_max')
            #     self.visualizer.add_image(f'{img_name[:-4]}_post_feature_map{i}', drawn_img_post)

            import torch
            mask_feature = recorder.data_buffer[0]  # remove the batch
            cls_logits = recorder.data_buffer[-2]
            mask_logits = recorder.data_buffer[-1]
            mask_pred = torch.einsum('bqc,bchw->bqhw', mask_logits, mask_feature)
            feature = torch.einsum('bqc, bqhw->bchw', cls_logits[..., :-1], mask_pred)[0]

            drawn_img_pre = self.visualizer.draw_featmap(
                feature, img_from_to[0], channel_reduction='select_max')
            self.visualizer.add_image(f'{img_name[:-4]}_pre_feature_map', drawn_img_pre)
            drawn_img_post = self.visualizer.draw_featmap(
                feature, img_from_to[1], channel_reduction='select_max')
            self.visualizer.add_image(f'{img_name[:-4]}_post_feature_map', drawn_img_post)
    ...
    def __call__(
        ...
        **kwargs,
        ...
        visualization = self.visualize(
            ori_inputs, preds, img_out_dir=img_out_dir, recorder=recoder,
