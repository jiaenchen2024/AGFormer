...
class PGFormerHead(MaskFormerHead):
    ...
    def __init__(self,
                 ...
                 loss_anchor_cls: ConfigType = dict(
                     ...
                     loss_weight=1.0,
                     reduction='none'),
                 loss_anchor_seg: ConfigType = dict(
                     type='mmseg.CrossEntropyLoss',
                     use_sigmoid=False,
                     loss_weight=0.01,
                     reduction='mean'),
        ...
        self.anchor_classes = torch.load('Estimated_prototypes/d256_e1000/C2/2centers_10anchor_num_256dim.pth')['w']
        ...
        # self.anchor_classes = nn.Parameter(torch.zeros(self.num_classes, self.num_anchors, feat_channels),
        #                                    requires_grad=True)
        ...
        self.proj = ProjectionHead(feat_channels, feat_channels)
        self.feat_norm = nn.LayerNorm(feat_channels)
        self.mask_norm = nn.LayerNorm(self.num_classes)

        self.update_flag = torch.tensor(0)
        self.update_conf = 0.9

        self.loss_anchor_cls = MODELS.build(loss_anchor_cls)
        self.loss_anchor_seg = MODELS.build(loss_anchor_seg)
    ...
    def prototype_learning(self, cls_pred_list, mask_pred_list, query_list, gt_seg, mask_features):
        loss_dict = {}

        center = self.anchor_classes.to(gt_seg.device)
        center = center.reshape(-1, self.feat_channels)
        center = self.projecter_linear_classes(center).unsqueeze(2).unsqueeze(2)
        ...
        # center_feat_avg = center_feat.mean(dim=1).squeeze(1)
        center_logits_classes = self.cls_embed(center_feat).squeeze(1)

        # anchor cls loss
        tgt_cls = (self.center_lbl.repeat_interleave(self.num_anchors)).to(gt_seg.device)
        ...
        if (right_pred_anchor.sum() == (self.num_classes * self.num_anchors)) and \
                (max(center_ce_mat.view(-1)) < -torch.log(torch.tensor(self.update_conf)).to(gt_seg.device)):
            if self.update_flag == 0:
                protos = l2_normalize(center_feat.data.clone()).to(gt_seg.device)
                self.update_flag = 1
            else:
                protos = l2_normalize(self.momentum_center_feat.data.clone()).to(gt_seg.device)

            _protos = protos.data.clone()
            seg_anchor_losses = []
            seg_losses = []
            ppd_loss = []
            for cls_pred, mask_pred, query in zip(cls_pred_list, mask_pred_list, query_list):
                # out_seg = torch.einsum("bqc,bqhw->bchw", cls_pred[..., :-1], mask_pred)
                seg_logits = torch.einsum("bqc,bqhw->bchw", query, mask_pred)
                b, c, h, w = seg_logits.shape
                _c = self.proj(seg_logits)
                _c = rearrange(_c, 'b c h w -> (b h w) c')
                _c = self.feat_norm(_c)
                _c = l2_normalize(_c)

                masks = torch.einsum('nd,kmd->nmk', _c, _protos)
                out_seg = torch.amax(masks, dim=1)
                out_seg = rearrange(out_seg, "(b h w) k -> b k h w", b=b, h=h)
                masks = masks.reshape(b, h, w, -1).permute(0, 3, 1, 2)
                _c = _c.reshape(b, h, w, c).permute(0, 3, 1, 2)

                with torch.no_grad():
                    points_coords_changed = get_uncertain_point_coords_with_randomness(
                        out_seg.sum(1).unsqueeze(1), None, self.num_points,
                        self.oversample_ratio, self.importance_sample_ratio)

                    point_labels = point_sample(
                        gt_seg,
                        points_coords_changed,
                        align_corners=False,
                    ).view(-1).long()
                ...
                masks = rearrange(masks, 'b (m k) n -> (b n) m k', m=self.num_anchors, k=self.num_classes)
                _c = rearrange(_c, 'b c n -> (b n) c')

                pred_seg = torch.max(point_logits, 1)[1]
                mask = (point_labels == pred_seg.view(-1))

                cosine_similarity = torch.mm(_c, _protos.reshape(-1, _protos.shape[-1]).t()
                                             .to(gt_seg.device))

                proto_logits = cosine_similarity
                proto_target = point_labels.clone().float()

                for k in range(self.num_classes):
                    init_q = masks[..., k]
                    init_q = init_q[point_labels == k, ...]
                    if init_q.shape[0] == 0:
                        continue

                    q, indexs = distributed_sinkhorn(init_q)

                    m_k = mask[point_labels == k]

                    c_k = _c[point_labels == k, ...]

                    m_k_tile = repeat(m_k, 'n -> n tile', tile=self.num_anchors)
                    ...
                    proto_target[point_labels == k] = indexs.float() + (self.num_anchors * k)

                self.momentum_center_feat = nn.Parameter(l2_normalize(protos),
                                                         requires_grad=False)

                seg_anchor_losses.append(self.loss_anchor_seg(proto_logits, proto_target.long()))
                seg_losses.append(F.cross_entropy(point_logits.permute(0, 2, 1).flatten(0, 1), point_labels.long()) * 0.1)
                logits = torch.gather(proto_logits, 1, proto_target[:, None].long())
                ppd_loss.append((1 - logits).pow(2).mean())
            seg_anchor_losses = sum(seg_anchor_losses) / len(seg_anchor_losses)
            ppd_loss = sum(ppd_loss) / len(ppd_loss)
            loss_dict['seg_anchor_loss'] = seg_anchor_losses
            loss_dict['seg_losses'] = seg_losses
            loss_dict['ppd_loss'] = ppd_loss
        return loss_dict
    ...
    def loss(self, x: Tuple[Tensor], batch_data_samples: SampleList,
        ...
        losses = {}
        loss_anchor = self.prototype_learning(all_cls_scores, all_mask_preds, all_queries, gt_seg, mask_features)
        losses.update(loss_anchor)
        # loss
